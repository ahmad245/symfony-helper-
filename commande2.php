...
//    }
     ...
     */
    public function exportPlanification(Request $request,$id=null,$annee=null,$mois=null,$commande=null)
    {
        ...
        if(is_null($commande)){
            ...
            $last=$em->getRepository('UfmcpBundle:Planification')->getLastCommande($id,$mois,$annee);

            $commande=$last['commande'];
            //  dump($commande);die;
        }
        $allCommandes=range(1,$commande);
           ...
           // $agenceDate= $preinscripteurs->getPlanificationsByCommande($allCommandes);
            ...
            $result=$em->getRepository('UfmcpBundle:Planification')-> findByMonthAndYearAgenda($mois,$annee,$id,$allCommandes);


            foreach ($result as $dateP)
            {
              $sumNumber=$dateP->getNombre();
               if(array_key_exists($dateP->getJourHeure()->format('d/m/Y H:i'),$datePrescripteur)){
                   $sumNumber=$datePrescripteur[$dateP->getJourHeure()->format('d/m/Y H:i')] + $dateP->getNombre() ;
               }
                $datePrescripteur[$dateP->getJourHeure()->format('d/m/Y H:i')]=$sumNumber;
            }
//        dump($datePrescripteur);
//        ;die;
            $organiseData=  $this->organiseData($semaines,$mois,$datePrescripteur,$result,$commande);
            $wArray= $organiseData[0];
            $jourOut=$organiseData[1];
            $territoireArray=$organiseData[2];
         ...
         //   var_dump($AllterritoireArray,$allAgenc,$allJourOut,$allNameOfAgance,$mois,$annee);die;
        ...
        foreach ($req->request->all() as $key=>$value){
           if($key != "agenda" && $key != "annee" && $key != "mois" &&$key !="commande"){
               if($value !== ""){
                ...
                //  dump($planificationExist,$value);die;

                //  if($agenceInf->isExist($planificationExist)){
                  if(count($planificationExist)>0){
                      $sum=0;
                      foreach ($planificationExist as $pl ){
                          $sum+=$pl->getNombre();
                      }
                      $nombre=(int)$value - $sum;

                      if((int)$value !== $sum && $nombre > 0 ){

                          $planification=new Planification();
                          $agenceInf->addPlanification($planification);
                          $planification->setJourHeure($oo);

                          $planification->setNombre($nombre);
...
//die;
        ...
        $last=$em->getRepository('UfmcpBundle:Planification')->getLastCommande($agenceInf->getId(),$mois,$annee);
        $commandes=range(1,$last['commande'] );
